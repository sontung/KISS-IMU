...
def run_sequence(seq: str, network: IMUNet, args) -> dict:
    ...
    init_pos = (
        ds.init["pos"].numpy() if torch.is_tensor(ds.init["pos"]) else ds.init["pos"]
    )
    init_rot = (
        ds.init["rot"].numpy() if torch.is_tensor(ds.init["rot"]) else ds.init["rot"]
    )
    ...
    anchor_vel = (
        init_vel.clone()
        if torch.is_tensor(init_vel)
        else torch.from_numpy(init_vel).float()
    )
    ...
    for sample in tqdm(dl, desc=f"inference[{seq}]"):
        ...
        zero_init = {
            ...
            "pos": torch.zeros((1, 3), device=args.device),
            "cov": None,
        }
        ...
        icp_poses, icp_motions, icp_overlap = lo_model(
            sample, pp.SE3(anchor_pose.to(args.device))
        )
    ...
    return {
        "imu_poses": (
            np.concatenate(imu_poses_all, axis=0) if imu_poses_all else np.zeros((0, 7))
        ),
        "icp_poses": (
            np.concatenate(icp_poses_all, axis=0) if icp_poses_all else np.zeros((0, 7))
        ),
        "pgo_poses": (
            np.concatenate(pgo_poses_all, axis=0) if pgo_poses_all else np.zeros((0, 7))
        ),
        "gt_poses": (
            np.concatenate(gt_poses_all, axis=0) if gt_poses_all else np.zeros((0, 7))
        ),
        "icp_overlap": (
            np.concatenate(icp_overlap_all, axis=0)
            if icp_overlap_all
            else np.zeros((0,))
        ),
        "pgo_overlap": (
            np.concatenate(pgo_overlap_all, axis=0)
            if pgo_overlap_all
            else np.zeros((0,))
        ),
    }
...
def main() -> int:
    ...
    for seq in args.seqs:
        ...
        print(
            f"[inference] {seq}: saved {npz_path}  "
