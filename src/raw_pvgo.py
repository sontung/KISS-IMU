...
def update_plot_dual(
    ax_xy, ax_xz, lines, icp_poses=None, pgo_poses=None, label_poses=None, gt_poses=None
):
...
def inference(lo_model, loader, integrator, data_seq, gravity):
    ...
    fig, (ax_xy, ax_xz) = plt.subplots(
        1, 2, figsize=(20, 8), dpi=120, constrained_layout=True
    )

    (gt_xy_line,) = ax_xy.plot([], [], "y--", label="GT", linewidth=6)
    (icp_xy_line,) = ax_xy.plot(
        [], [], "r-", label=f"ICP({args.lo_model})", linewidth=3
    )
    ...
    lines = {
        ...
        "icp_xy": icp_xy_line,
        "pgo_xy": pgo_xy_line,
        "label_xy": label_xy_line,
        "gt_xz": gt_xz_line,
        "icp_xz": icp_xz_line,
        "pgo_xz": pgo_xz_line,
        "label_xz": label_xz_line,
    }
    ...
    with torch.no_grad():
        for i, sample in enumerate(tqdm.tqdm(loader)):
            if isinstance(inference_poses_list[-1], torch.Tensor):
                ...
                init_pos = (
                    torch.from_numpy(inference_poses_list[-1][:3])
                    .to(args.device)
                    .float()
                )
                init_rot = (
            ...
            valid = (
                torch.as_tensor(valid, device=imu_dts.device).flatten().to(torch.long)
            )
            T = imu_dts.size(-1)
            ...
            imu_states = integrator.integrate(
                init=init_state,
                dts=corr_data["dts"],
                accels=corr_data["accels"],
                gyros=corr_data["gyros"],
                motion_mode=False,
            )
            ...
            icp_poses, icp_motions, icp_overlap_scores = lo_model(
                sample, pp.SE3(inference_poses_list[-1])
            )

            if args.use_adaptive_weight:
                pgo_poses, _ = optimize(
                    nodes=imu_nodes,
                    vels=imu_vels,
            ...
            imu_poses = pp.SE3(
                torch.cat(
                    [imu_states["pos"][1:, :], imu_states["rot"].tensor()[1:, :]],
                    dim=-1,
                )
            ).to(args.device)
            inference_loss = get_valid_losses(
                imu_poses, imu_vels[1:], gt_poses, gt_vels, args
            )
            ...
            pgo_pose = (
                np.stack([t.detach().cpu() for t in pgo_poses_list])
                if pgo_poses_list
                else np.array([])
            )
            icp_pose = (
                np.stack([t.detach().cpu() for t in icp_poses_list])
                if icp_poses_list
                else np.array([])
            )
            gt_pose = (
...
def plot_pose_trajs(icp_poses, pgo_poses, label_poses, gt_poses, filename):
    ...
    if gt_poses is not None and len(gt_poses) > 0:
        gt_np = np.asarray(gt_poses)
        ax.plot(
            gt_np[:, 0], gt_np[:, 1], "k--", label="Ground Truth", linewidth=3, zorder=1
        )
    if icp_poses is not None and len(icp_poses) > 0:
        icp_np = np.asarray(icp_poses)
        ax.plot(
    ...
    if pgo_poses is not None and len(pgo_poses) > 0:
        pgo_np = np.asarray(pgo_poses)
        ax.plot(
            pgo_np[:, 0], pgo_np[:, 1], "b-", label="PGO Pose", linewidth=2.5, zorder=3
        )
    if label_poses is not None and len(label_poses) > 0:
        label_np = np.asarray(label_poses)
        ax.plot(
...
if __name__ == "__main__":
    ...
    for inference_seq in args.inference_seqs:
        ...
        inference_dataset = SeqDataset(
            data_root=args.data_root,
            data_seq=inference_seq,
        ...
        lo_model = LOModule(
            ...
            T_I_L=inference_dataset.T_I_G,
            R_I_L=inference_dataset.R_I_L,
            init_state=icp_poses_list[-1],
            device_id=args.device,
        )
        integrator = IMUIntegrator(
            init_state=inference_dataset.init, device=args.device
        )

        inference_loss = inference(
            lo_model=lo_model,
            loader=inference_loader,
            integrator=integrator,
            data_seq=inference_seq,
            gravity=inference_dataset.gravity,
        )
