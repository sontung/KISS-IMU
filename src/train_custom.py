...
class FreqGate:
    ...
    def keep_mask(self, comp_ids: torch.Tensor):
        ...
        freq = (self.rate + self.prior) / (
            (self.rate.sum() + self.prior * self.K) + self._warm
        )
        target = 1.0 / float(self.K)

        with torch.no_grad():
            p_keep_per_comp = torch.clamp(
                (target / (freq + self._warm)) ** self.alpha,
                min=self.p_min,
                max=self.p_max,
            )
...
def train(
    ...
    for i, sample in enumerate(tqdm.tqdm(loader, total=max_iterations)):
        ...
        if len(label_poses_list) > 0:
            if isinstance(label_poses_list[-1], torch.Tensor):
                pose_vec = (
                    label_poses_list[-1][0]
                    if label_poses_list[-1].ndim == 2
                    else label_poses_list[-1]
                )
                ...
                pose_vec = (
                    label_poses_list[-1][0]
                    if len(label_poses_list[-1].shape) == 2
                    else label_poses_list[-1]
                )
        ...
        imu_states = integrator.integrate(
            init=init_state,
            dts=corr_data["dts"],
            accels=corr_data["accels_corr"],
            gyros=corr_data["gyros_corr"],
            cov_accels=corr_data["acc_cov"],
            cov_gyros=corr_data["gyr_cov"],
            motion_mode=False,
        )
        ...
        imu_nodes = pp.SE3(
            torch.cat([imu_states["pos"], imu_states["rot"].tensor()], dim=-1)
        ).to(args.device)
        imu_vels = imu_states["vel"].to(args.device)
        imu_motions = pp.SE3(
            torch.cat([imu_dstates["pos"], imu_dstates["rot"].tensor()], dim=-1)
        ).to(args.device)
        ...
        if getattr(args, "use_gt", False):
            ...
            icp_poses, icp_motions, icp_overlap_scores = lo_model(
                sample, pp.SE3(label_poses_list[-1])
            )
            ...
            pgo_poses, pgo_vels = optimize(
                ...
                imu_drots=imu_dstates["rot"],
                imu_dvels=imu_dstates["vel"],
                imu_dtrans=imu_dstates["pos"],
                imu_dts=imu_dts,
            ...
            pgo_overlap_scores = batch_pose_aligned_overlap(
                lo_model.scans0_np, lo_model.scans1_np, pgo_motions
            )

            better_idx = torch.from_numpy(
                np.argmax(np.stack([icp_overlap_scores, pgo_overlap_scores]), axis=0)
            ).to(icp_poses.device)
            ...
            motions_stack = torch.stack(
                [icp_motions.tensor(), pgo_motions.tensor()], dim=0
            )
        ...
        label_vels = torch.cat(
            [init_vel.unsqueeze(0).detach(), label_vels.detach()], dim=0
        )
        label_dvels = label_vels[1:] - label_vels[:-1]
        label_motions = pp.SE3(label_motions)

        rot_loss_v, vel_loss_v, pos_loss_v, rot_cov_v, vel_cov_v, pos_cov_v = (
            get_losses(
                imu_nodes,
                imu_vels,
                imu_motions,
        ...
        if global_step % 10 == 0 or global_step == 1:
            for name, tensor in [
                ("rot_loss_v", rot_loss_v),
                ("vel_loss_v", vel_loss_v),
                ("pos_loss_v", pos_loss_v),
                ("rot_cov_v", rot_cov_v),
                ("vel_cov_v", vel_cov_v),
                ("pos_cov_v", pos_cov_v),
            ]:
        ...
        comp_ids_t = torch.as_tensor(
            comp_ids, device=rot_loss_v.device, dtype=torch.long
        )
        invalid_mask = (comp_ids_t < 0) | (comp_ids_t >= K)
        if invalid_mask.any():
            if (global_step % 10 == 0) or (global_step == 1):
                monitor.writer.add_scalar(
                    "train_gmm/invalid_comp_ids", invalid_mask.sum().item(), global_step
                )
        ...
        if gmm is not None:
            base_w = gmm.torch_weights_from_labels(
                np.array(comp_ids_list),
                device=rot_loss_v.device,
                dtype=rot_loss_v.dtype,
            )
        else:
        ...
        if conv_mgr is not None:
            active_mask_np = np.array(
                [not conv_mgr.is_converged(k) for k in comp_ids_list], dtype=np.float32
            )
            active_mask = torch.from_numpy(active_mask_np).to(
                rot_loss_v.device, rot_loss_v.dtype
            )
            if conv_mgr.all_converged(comp_ids_list):
                print(
                    f"\033[92m[INFO] batch skipped (all components converged: {sorted(set(comp_ids_list))})\033[0m"
                )
                continue
        else:
        ...
        if hasattr(train, "_freq_gate") and gmm is not None:
            ...
            if global_step % 10 == 0 or global_step == 1:
                monitor.writer.add_scalar(
                    "train_gmm/keep_prob_mean", p_keep.mean().item(), global_step
                )
                monitor.writer.add_scalar(
                    "train_gmm/keep_rate_batch", keep_mask.mean().item(), global_step
                )
        else:
        ...
        per_sample_total = (
            args.rot_w * rot_loss_v
            + args.cov_r_w * rot_cov_v
            + args.vel_w * vel_loss_v
            + args.cov_v_w * vel_cov_v
            + args.pos_w * pos_loss_v
            + args.cov_t_w * pos_cov_v
        )

        if global_step % 10 == 0 or global_step == 1:
            if (
                torch.isnan(per_sample_total).any()
                or torch.isinf(per_sample_total).any()
            ):
                ...
                per_sample_total = torch.where(
                    torch.isnan(per_sample_total) | torch.isinf(per_sample_total),
                    torch.zeros_like(per_sample_total),
                    per_sample_total,
                )
        ...
        if hasattr(train, "_freq_gate") and gmm is not None:
            with torch.no_grad():
                freq = (train._freq_gate.rate + train._freq_gate.prior) / (
                    train._freq_gate.rate.sum()
                    + train._freq_gate.prior * train._freq_gate.K
                    + 1e-6
                )
                target = 1.0 / float(train._freq_gate.K)
                rarity = torch.clamp(
                    (target / (freq + 1e-6)) ** train._freq_gate.alpha,
                    min=train._freq_gate.p_min,
                    max=train._freq_gate.p_max,
                )
        ...
        if global_step % 10 == 0 or global_step == 1:
            ...
            log_pose_metrics(
                ...
                cov_losses={
                    "total": rot_cov_log + vel_cov_log + pos_cov_log,
                    "rot": rot_cov_log,
                    "vel": vel_cov_log,
                    "pos": pos_cov_log,
                },
            ...
            if len(pgo_poses_list) > 1 and len(gt_poses_list) > 1:
                pgo_poses_np = np.stack(
                    [t.detach().cpu().numpy() for t in pgo_poses_list]
                )
                gt_poses_np = np.stack(
                    [t.detach().cpu().numpy() for t in gt_poses_list]
                )
                poses_dict = {"gt": gt_poses_np, "pgo": pgo_poses_np}
                if len(icp_poses_list) > 1:
                    icp_poses_np = np.stack(
                        [t.detach().cpu().numpy() for t in icp_poses_list]
                    )
                    poses_dict["icp"] = icp_poses_np
                if len(label_poses_list) > 1:
                    label_poses_np = np.stack(
                        [t.detach().cpu().numpy() for t in label_poses_list]
                    )
                ...
                monitor.log_multiple_pose_comparison(
                    poses_dict,
                    global_step,
                    f"train_all_trajectories_{data_seq}",
                    epoch=epoch_i,
                    train_valid="train",
                )
                ...
                try:
                    ...
                    if comp_stats["component_usage"]:
                        component_ids = np.array(
                            sorted(comp_stats["component_usage"].keys())
                        )
                        usage_counts = np.array(
                            [
                                comp_stats["component_usage"][cid]
                        ...
                        ax.set_ylabel("# used in optimize")
                        ax.set_xticks(component_ids)
                        ax.grid(True, axis="y", alpha=0.3)
                        monitor.writer.add_figure(
                            "train_gmm/usage_bar", fig, global_step
                        )
                        plt.close(fig)
    ...
    if icp_count + pgo_count > 0:
        ...
        monitor.writer.add_scalar(
            f"pose_selection/{data_seq}_icp_ratio", icp_count / total_count, epoch_i
        )
        monitor.writer.add_scalar(
            f"pose_selection/{data_seq}_pgo_ratio", pgo_count / total_count, epoch_i
        )
        monitor.writer.add_scalar(
            f"pose_selection/{data_seq}_icp_count", icp_count, epoch_i
        )
        monitor.writer.add_scalar(
            f"pose_selection/{data_seq}_pgo_count", pgo_count, epoch_i
        )
...
def validate(data_loader, network, integrator, data_seq, epoch_i):
    ...
    for sample in tqdm.tqdm(data_loader):
        ...
        pred_last_se3 = pp.SE3(
            torch.cat([out_state["pos"], out_state["rot"].tensor()], dim=-1)
        ).to(device)
    ...
    print("====================================================")
    print(
        f"[Summary] RTE in {data_seq} at epoch {epoch_i}: {mean_ep_rpe_trans:.3f} m, "
    )
    ...
    try:
        ...
        if icp_poses is not None and len(icp_poses) > 0:
            ...
            ax.plot(
                ...
                icp_np[:, 1],
                "r-",
                label=f"ICP({args.lo_model}) Pose",
                linewidth=2.5,
                zorder=2,
            )
    ...
    try:
        ...
        if total_updates > 0:
            ...
            ax2.pie(
                ...
                labels=[
                    f"Comp {cid}\n({p:.1f}%)"
                    for cid, p in zip(component_ids, percentages)
                ],
                autopct="%1.1f%%",
                startangle=90,
                colors=colors,
...
def save_gmm_initial_histogram(gmm, result_dir):
    ...
    try:
        ...
        bars = ax1.bar(
            component_ids, component_weights, alpha=0.7, color=colors, edgecolor="black"
        )
        ax1.set_xlabel("Component ID")
        ...
        if component_means.shape[1] >= 3:
            ...
            for i, (x, y) in enumerate(
                zip(component_means[:, 0], component_means[:, 1])
            ):
                ax3.annotate(
                    f"C{i}",
                    (x, y),
        ...
        fig.suptitle(
            f"Initial GMM Component Distribution\n{n_components} Components",
            fontsize=16,
        )
        plt.tight_layout()
...
def create_gmm_data_packs(train_packs, train_ratio):
    ...
    for seq, dataset, loader in train_packs:
        ...
        print(
            f"  * GMM data for {seq}: {total_samples} -> {limited_samples} samples (ratio: {train_ratio:.2f})"
        )
...
if __name__ == "__main__":
    ...
    if args.scheduler == "ReduceLROnPlateau":
        ...
        scheduler = StepLR(
            optimizer, step_size=args.scheduler_step_size, gamma=args.scheduler_factor
        )
    elif args.scheduler == "CosineAnnealingLR":
        scheduler = CosineAnnealingLR(
            optimizer, T_max=args.epoch, eta_min=args.scheduler_min_lr
        )
    else:
    ...
    for epoch_i in range(1, args.epoch + 1):
        ...
        for train_seq, train_dataset, train_loader in train_packs:
            ...
            if args.use_submap:
                lo_model = LOModule(
                    lo_model=args.lo_model,
                    T_I_L=train_dataset.T_I_G,
                    R_I_L=train_dataset.R_I_L,
                    init_state=icp_poses_list[-1],
            ...
            integrator = IMUIntegrator(
                init_state=train_dataset.init, device=args.device
            )
            ...
            gt_pose = (
                np.stack([t.detach().cpu() for t in gt_poses_list])
                if gt_poses_list
                else np.array([])
            )
            plot_pose_trajs(
        ...
        print(
            f"  * Total Train Loss in {epoch_i} epoch: {epoch_train_loss / len(args.train_seqs)} *   "
        )
        ...
        monitor.log_epoch_summary(
            ...
            val_losses=(
                {"total_loss": epoch_valid_loss / len(args.valid_seqs)}
                if args.valid_seqs
                else None
            ),
        )
        ...
        for valid_seq, valid_dataset, valid_loader in valid_packs:
            ...
            integrator = IMUIntegrator(
                init_state=valid_dataset.init, device=args.device
            )
        ...
        print(
            f"  * Total Valid Loss in {epoch_i} epoch: {epoch_valid_loss / len(args.valid_seqs)} *   "
        )
