...
def eval_state(data_loader, integrator, network, device, save_plot_path=None):
    ...
    if save_plot_path is not None and len(pred_last) > 0:
        ...
        plt.ylabel("y [m]")
        plt.legend()
        plt.title("Endpoint positions per window (XY)")
        plt.tight_layout()
...
def evaluate_ckpt(ckpt_path, args):
    ...
    for seq in args.eval_seqs:
        ...
        integrator = IMUIntegrator(
            init_state=ds.init, prop_cov=True, gravity=ds.gravity, device=args.device
        )
        plot_path = (
            os.path.join(args.result_dir, f"{ckpt_tag}_{seq}_xy.png")
            if args.save_plot
            else None
        )
    ...
    avg = {
        k: float(np.mean([per_seq[s][k] for s in args.eval_seqs]))
...
def main():
    ...
    for seq in args.eval_seqs:
        m = per_seq[seq]
        print(
            f"  {seq:>20s}  rpe_trans={m['rpe_trans']:.3f} m | "
